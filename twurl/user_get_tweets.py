...
def get_user_tweets(destination_dir, dir_number, user_handle):
	...
	destination_dir = "twitter_data/"  + str(destination_dir) + "/" + str(dir_number).zfill(5) + "/"
...
curr_id = -1
maxID = 99999999999999999999999999999999999999999999999999999999999999999999
while curr_id > -2:
	for tweet in tweepy.Cursor(api.user_timeline,screen_name="TrustamW", tweet_mode='extended', wait_on_rate_limit=True, max_id=maxID).items():
			#print(tweet)
			#print(tweet.text)
			#print(tweet.favorite_count)
			curr_id = tweet.id
			if (not tweet.retweeted) and ('RT @' not in tweet.full_text):
				count = count + 1
				print("Date of Tweet: ", tweet.created_at)
				totalFav = totalFav + tweet.favorite_count
				tweetfilename = dir + str(count).zfill(5) + ".txt"
				with open(tweetfilename, 'a', encoding="utf-8") as file:
					#print(tweet.text)
